function [time_ratio,Signal_Ca_ratio,sub_output_folder]=Convert_raw_data_to_ratio_sub(folder_name,input_file,output_folder_name)
...
file_to_read=[folder_name,'/',input_file];
...
for c=1:number_cells
    ...
    subplot(2,3,1),plot(Signal_Ca_ratio_340_380(:,c),'ko-','MarkerFaceColor','g','LineWidth',1);
    subplot(2,3,1),title('1st/2nd');
    xlim([0 500]);
    Signal_Ca_ratio_380_340(:,c)=Signal_Ca_380_channel(:,c)./Signal_Ca_340_channel(:,c);
    subplot(2,3,4),plot(Signal_Ca_ratio_380_340(:,c),'ko-','MarkerFaceColor','g','LineWidth',1);
    subplot(2,3,4), title('2nd/1st');
    xlim([0 500]);
    % Smooth the signal
    ...
    % Calculate sloop of the signal
    Signal_Ca_ratio_diff_340_380(:,c)=diff(Signal_Ca_ratio_340_380_tmp)/time_ratio_diff(1);
    subplot(2,3,2),plot(Signal_Ca_ratio_diff_340_380(:,c),'ko-','MarkerFaceColor','g','LineWidth',1);
    subplot(2,3,2),title('340/380 derivatives');
    xlim([0 500]);
    Signal_Ca_ratio_diff_380_340(:,c)=diff(Signal_Ca_ratio_380_340_tmp)/time_ratio_diff(1);
    subplot(2,3,5),plot(Signal_Ca_ratio_diff_380_340(:,c),'ko-','MarkerFaceColor','g','LineWidth',1);
    subplot(2,3,5), title('380/340 derivatives');
    xlim([0 500]);
    if c==1
    ...
    subplot(2,3,3),plot(Signal_Ca_ratio(:,c),'k-','LineWidth',1);
    subplot(2,3,3), title([input_file,' Corrected Ratio for Cell ',num2str(c)]);
    xlim([0 500]);
    %
    fprintf(' ## Corrected ratio has been calculated for cell No. %d\n',c);
    output_screenshots=[output_folder_name,'/Signal_Cell_Number_',num2str(c)];
